...
def show_results(img, xyxy, conf, landmarks, class_num):
    ...
    cv2.line(img,(point[0],point[1]),(point[2],point[3]),(0,255,0),3)
    cv2.line(img,(point[2],point[3]),(point[4],point[5]),(0,255,0),3)
    cv2.line(img,(point[4],point[5]),(point[6],point[7]),(0,255,0),3)
    cv2.line(img,(point[6],point[7]),(point[0],point[1]),(0,255,0),3)
    #print(point)
    

    tf = max(tl - 1, 1)  # font thickness
    label = str(conf)[:5]
    #cv2.putText(img, "class:"+str(names[str(int(float(class_num)))]), (x1+50, y1 - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
    cv2.putText(img, str(names[str(int(float(class_num)))]), (x1, y1 - 2), 0, tl / 3, [0, 255, 0], thickness=tf, lineType=cv2.LINE_AA)
    return img
...
if __name__ == '__main__':
    ...
    detect(model, source, device, project, name, False, True, True)
